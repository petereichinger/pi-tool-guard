...
- When running in a [Herdr](https://herdr.dev) pane, Tool Guard bridges pi's native `ui_prompt_start` / `ui_prompt_end` lifecycle to the Herdr pi integration, which reports active guard dialogs as waiting for input. Tool Guard does not invoke `herdr pane report-agent` or maintain a separate Herdr state source. Herdr remains optional.
